...
public class Calculation extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Bundle bundle5 = intent.getBundleExtra("attritruss");
        Bundle bundle6 = intent.getBundleExtra("moment");
        ...
        ArrayList<Map<String,ArrayList>> momentS = (ArrayList<Map<String, ArrayList>>) bundle6.getSerializable("moment");

        ArrayList<ArrayList> attritrussS = (ArrayList<ArrayList>) bundle5.getSerializable("attritruss");
        ...
        Map<String, ArrayList> momentmapS = new HashMap<>();
        for (int i = 0; i < momentS.size(); i++) {
            momentmapS = momentS.get(i);
        }
        Log.w("momentS", momentmapS.toString());
        ...
        int count = 0;
        for (int i = 0; i < n; i++) {
            PointFreedom[i][0] = (int)supportmapS.get(String.valueOf(checkn.get(i))).get(1);
            if (PointFreedom[i][0] == 1){
                count++;
            }
            PointFreedom[i][1] = (int)supportmapS.get(String.valueOf(checkn.get(i))).get(2);
            ...
            F:for(int j=z1; j < PointFreedom.length;j++){
                for(int k=z;k < 2;k++){
                    if(PointFreedom[j][k] == 1){
                        if(k == 0){
                            FM[i][0] = (float)forcemapS.get(String.valueOf(j)).get(1);
                            z=z+1;
                        }
                        if(k == 1){
                            FM[i][0] = (float)momentmapS.get(String.valueOf(j)).get(0);
                            z1=z1+1;
                            z=0;
                        }
        ...
        ArrayList<ArrayList> LengthM =new ArrayList<>();
            ...
            for (int k = 0; k < m; k++) {
                ...
                float EI =(float)attritrussS.get(k).get(0)*(float)attritrussS.get(k).get(2);
                Log.w("EI",EI+",");
                /*=====填矩陣=====*/
            ...
            */
            ...
            for (int i = 0; i < Q.length; i++) {
                ...
                BigDecimal anser = new BigDecimal(Q[i][0]);
                BigDecimal ansershow =anser.setScale(3, RoundingMode.HALF_UP);
                if (num == 1) {
                    calaview.setText("Node" + newi + "    Y : " + calaview.getText().toString() + ansershow + "kN");
                } else if (num == 2) {
                    calaview.setText("Node" + newi + "    M : " + calaview.getText().toString() + ansershow + "N*m" + "\n");
                    num = 0;
                    newi++;
                }
                calaroot.addView(calaview);
